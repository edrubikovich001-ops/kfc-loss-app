...
/**
 * ENV
 * DATABASE_URL  - строка подключения Supabase Postgres (ОБЯЗАТЕЛЬНО Session Pooler)
...
const pool = new Pool({
  ...
  // Supabase требует SSL; для Render ок вот так.
...
async function ensureSchema() {
  // request_id UNIQUE — защита от дублей
  await q(`
...
// schema init (если БД недоступна — не валим весь сервер, чтобы ты видел ошибку через /api/health)
...
app.get("/api/health", async (req, res) => {
  try {
    ...
    res.json({ ok: true, schemaReady });
  } catch (e) {
    res.status(500).json({ ok: false, schemaReady, error: e?.message || "db error" });
  }
});
...
app.post("/api/reports", async (req, res) => {
  try {
    ...
    // Если запрос повторился — запись не задублируется.
    await q(
      `
...
// export excel — НУЖНЫЕ КОЛОНКИ + ФОРМАТ ТЕНГЕ + ШИРИНЫ
app.get("/api/export.xlsx", async (req, res) => {
  try {
    ...
    // Формат суммы: последняя колонка (индекс 7)
    if (ws["!ref"]) {
...
// SPA fallback (Telegram WebApp может приходить с любыми путями)
app.get(/^\/(?!api).*/, (req, res) => {
